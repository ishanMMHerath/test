      ...
      <vue-good-table :columns="columns" styleClass="vgt-table bordered centered" :rows="paginatedData"
        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field === 'vehicle'">
            <template v-if="props.row.vehicle.length">
              <a class="text-blue-700 font-bold" title="Click to view"
                :href="'https://yokodrive.com/vehicle/' + props.row.vehicle[0].vehicle_id" target="_blank">
                {{ props.row.vehicle[0].vehicle_name }}</a>
            </template>
            <template v-else>
              N/A
            </template>
          ...
          <span v-if="props.column.field === 'message'">
            <template v-if="props.row.message">
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get("inquiries/get-all");
