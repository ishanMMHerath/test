...
export default {
  ...
  methods: {
    async saveMasterDetail() {
      if (!this.form.name || !this.form.page_url || this.form.status === null) {
        Swal.fire({
          icon: 'warning',
          title: 'Warning',
      ...
      try {
        ...
        const response = await axios.post('pages/save',{
        ...
        if (response.status === 201) { // Check for successful status code
          ...
          router.push('/app/list-pages'); // Redirect to body type list
        } else {
          Swal.fire({
            icon: 'error',
            title: 'Error',
