      ...
      <div v-if="advancedTableSearch && sections.length" class="fromGroup flex gap-2">
        ...
        <select
          ...
          <option disabled value="">Filter by Status</option>
            <option value="all">All</option>
          <option style="text-transform: capitalize;" v-for="(item,i) of inventory_status_arr" :key="i"  :value="item.name.toLowerCase()">{{ item.name.toLowerCase() }}</option>
      ...
      <div v-if="advancedTableSearch && sections.length">
        ...
        selectAllByGroup: true,
          ...
          <template v-slot:table-row="props">
            ...
            <!-- <span v-if="props.column.field == 'min_payment'" class="text-sm text-slate-600 dark:text-slate-300">
          ...
          <template #pagination-bottom="props">
            <div class="py-4 px-3">
              <Pagination :current="current" :options="options" :pageChanged="props.pageChanged" :pageRange="pageRange"
                :per-page="perpage" :perPageChanged="props.perPageChanged" :total="totalRows" enableSearch enableSelect
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get("vehicle/draft-list");
       ...
       const vehicles  = response.data.map(vehicle => {
          ...
          return {
            ...vehicle,
            featured_image: featuredImage ? `${ImageUrl}${featuredImage.name}` : '',
