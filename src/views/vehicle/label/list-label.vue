        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'action'" class="flex space-x-1">
            <Button
                ...
                @click="handleAction('view', props.row.vehicle_labels_id)"
            />
            <Button
                icon="heroicons-outline:pencil-square"
                btnClass="btn-outline-warning btn-sm"
                @click="handleAction('edit', props.row.vehicle_labels_id)"
            />

            <Button
                icon="heroicons-outline:trash"
                btnClass="btn-outline-danger btn-sm"
                @click="handleAction('delete', props.row.vehicle_labels_id)"
            />
          </span>
        </template>
      ...
      if (action === "delete") {
        try {
          ...
          if (result.isConfirmed) {
            await axios.delete(`vehicle-label/destroy/${id}`);
        ...
        this.$router.push({name: 'edit-label', params: {id: id}});
