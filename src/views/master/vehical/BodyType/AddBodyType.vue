...
import {events as eventBus} from "vuedraggable/src/core/sortableEvents"; // Ensure your router is imported
  ...
  methods: {
    async saveMasterDetail() {
      ...
      try {
        ...
        const response = await axios.post('body-type/save', {
          name: this.form.name,
        ...
        if (response.status === 201) {
          ...
          router.push('/app/list-body-type'); // Redirect to body type list
        } else {
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Failed to save body type. Please try again.',
          });
        }
      } catch (error) {
