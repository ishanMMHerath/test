<template>
  <div>
    <Card noborder>
      ...
      >
        <h5>
          <router-link to="/app/add-inventory-status">
            <Button
                icon="heroicons-outline:newspaper"
                text="Add InventoryStatus"
        ...
        <template v-slot:table-row="props">
          ...
          <span class="justify-center flex space-x-1" v-if="props.column.field == 'action'">
            ...
            <Button
                ...
                @click="handleAction('view', props.row.inventory_statuses_id)"
            />
            <Button
                icon="heroicons-outline:pencil-square"
                btnClass="btn-outline-warning btn-sm"
                @click="handleAction('edit', props.row.inventory_statuses_id)"
            />
         
            <Button
                icon="heroicons-outline:trash"
                btnClass="btn-outline-danger btn-sm"
                @click="handleAction('delete', props.row.inventory_statuses_id)"
            />
            ...
            <Dropdown classMenuItems="w-[140px]">
              ...
              <template v-slot:menus>
                <MenuItem v-for="(item, i) in actions" :key="i">
                  <div
                      @click="handleAction(item.name, props.row.inventory_statuses_id)"
                      :class="`
                      ${
                        item.name === 'delete'
...
export default {
  ...
  methods: {
    ...
    async handleAction(action, id) {
      ...
      //Edit Action
      if(action === "edit") {
           this.$router.push({name: 'edit-inventory-status', params: {id: id}});
      }
      ...
      if (action === "delete") {
        try {
          ...
          if (result.isConfirmed) {
            await axios.delete(`inventory-status/destroy/${id}`);
