          ...
          <span v-if="props.column.field == 'status'" class="block w-full">
            ...
            <span class="justify-center flex space-x-1" v-if="props.column.field == 'action'">
              ...
              <Button
                  ...
                  @click="handleAction('view', props.row.years_id)"
              />
              <Button
                  icon="heroicons-outline:pencil-square"
                  btnClass="btn-outline-warning btn-sm"
                  @click="handleAction('edit', props.row.years_id)"
              />
           
              <Button
                  icon="heroicons-outline:trash"
                  btnClass="btn-outline-danger btn-sm"
                  @click="handleAction('delete', props.row.years_id)"
              />
...
export default {
  ...
  methods: {
    ...
    async handleAction(action, id) {
       ...
       //Edit Action
      if(action === "edit") {
           this.$router.push({name: 'edit-years', params: {id: id}});
      }
      ...
      if (action === "delete") {
        try {
          ...
          if (result.isConfirmed) {
            await axios.delete(`year/destroy/${id}`);
