  ...
  methods: {
    async saveMasterDetail() {
      ...
      try {
        ...
        const response = await axios.post('jci-status/save',{
          name: this.form.name,
        ...
        if (response.status === 201) { // Check for successful status code
          ...
          router.push('/app/list-jci-status'); // Redirect to body type list
        } else {
          Swal.fire({
            icon: 'error',
            title: 'Error',
        ...
        console.error('Error saving jci-status Modal:', error);
        Swal.fire({
          icon: 'error',
          title: 'Error',
