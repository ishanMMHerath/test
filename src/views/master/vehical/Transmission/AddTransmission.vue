  ...
  methods: {
    async saveMasterDetail() {
      ...
      try {
        ...
        const response = await axios.post('transmission/save',{
          name: this.form.name,
        ...
        if (response.status === 201) { // Check for successful status code
          ...
          router.push('/app/list-transmission'); // Redirect to body type list
        } else {
          Swal.fire({
            icon: 'error',
            title: 'Error',
        ...
        console.error('Error saving transmission Modal:', error);
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Failed to save transmission. Please try again.',
        });
      }
    },
  },
